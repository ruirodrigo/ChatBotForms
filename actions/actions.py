...
ALLOWED_CLASSE = ["ECO", "BUS", "PEC", "PST"]
ALLOWED_ITINERARIO = ["ONE_WAY", "ROUND_TRIP"]

class ValidateSimplePassagemForm(FormValidationAction):
  def name(self) -> Text:
    return "validatesimplepassagemform"
    
  #nome do form: simple_passagem_form
  ...
  def validate_classe(
    ...
    domain: DomainDict,
      ...
      if slot_value.lower() not in ALLOWED_CLASSE:
          dispatcher.utter_message(text=f"Somente aceitamos classes: ECO/BUS/PEC/PST.")
          return {"Classe": None}
      dispatcher.utter_message(text=f"OK! Você deseja a classe {slot_value}.")
      return {"Classe": slot_value}

  def validate_itinerario(
    self,
    slot_value: Any,
    dispatcher: CollectingDispatcher,
    tracker: Tracker,
    domain: DomainDict,
      ...
      if slot_value.lower() not in ALLOWED_ITINERARIO:
          dispatcher.utter_message(text=f"Somente aceitamos Itinerario: ONE_WAY/ROUND_TRIP.")
          return {"Itinerario": None}
      dispatcher.utter_message(text=f"OK! Você deseja o itinerario {slot_value}.")
      return {"Itinerario": slot_value}


class ActionBuscaPassagem(Action):

    def name(self) -> Text:
        return "action_buscar_passagens"
